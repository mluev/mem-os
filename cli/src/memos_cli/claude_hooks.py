...
def repo_settings(cwd: Path | str) -> Repo:
    ...
    return Repo(
        ...
        recall=bool(section.get("recall", True)) or os.environ.get("MEMKIT_RECALL") == "1",
    )
...
from .capture import events_from_delta
...
def _send(client: Any, events: list[dict[str, Any]], *, timeout=CAPTURE_TIMEOUT) -> None:
    try:
        ...
        client.post("/v1/evidence/events:batch", {"events": private}, timeout=timeout)
