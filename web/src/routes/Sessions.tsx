...
import type { Message, OffsetPage, Session } from "../api/types";
import { Badge, Card, EmptyState, ErrorState, Loading } from "../components/ui";
import { relativeTime } from "../lib/format";

export function Sessions() {
  const query = useQuery({ queryKey: ["sessions"], queryFn: () => api<OffsetPage<Session>>("/v1/admin/sessions?limit=100") });
  return <><div className="page-header"><div><span className="eyebrow">SOURCE EVIDENCE</span><h1>Sessions</h1><p>Normalized, redacted evidence retained for replay and provenance.</p></div></div><Card>{query.isLoading ? <Loading /> : query.error ? <ErrorState error={query.error} /> : !query.data?.items.length ? <EmptyState title="No sessions" body="Evidence events create sessions automatically." /> : query.data.items.map((session) => <Link className="list-row" key={session.id} to="/sessions/$sessionId" params={{ sessionId: session.id }}><span className="list-primary"><strong>{session.id}</strong><span>{session.agent_id} · {relativeTime(session.started_at)}</span></span><Badge>{session.message_count} messages</Badge><Badge>{session.unprocessed_count} pending</Badge><ChevronRight size={15} /></Link>)}</Card></>;
}

interface MessagePage extends OffsetPage<Message> { session: Session }
export function SessionDetail() {
  const { sessionId } = useParams({ strict: false }) as { sessionId: string };
  const query = useQuery({ queryKey: ["session", sessionId], queryFn: () => api<MessagePage>(`/v1/admin/sessions/${encodeURIComponent(sessionId)}/messages?limit=500`) });
