...
import type { JudgeRun, JudgeRuns as JudgeRunsResponse } from "../api/types";
...
export function JudgeRuns() {
  const query = useQuery({ queryKey: ["judge-runs"], queryFn: () => api<JudgeRunsResponse>("/v1/admin/judge-runs?limit=100") });
...
export function JudgeRunDetail() {
  ...
  return <><div className="page-header"><div><Link to="/judge-runs" className="eyebrow"><ArrowLeft size={11} /> JUDGE RUNS</Link><h1>Run #{run.id}</h1><p>{run.prompt_version} · {run.model}</p></div></div>{run.error ? <div className="health-banner"><strong>{run.error}</strong></div> : null}<div className="detail-grid"><Card><div className="card-header"><h2>Redacted input</h2></div><pre className="raw-json">{pretty(run.input)}</pre></Card><Card><div className="card-header"><h2>Structured output</h2></div><pre className="raw-json">{pretty(run.output)}</pre></Card></div></>;
}
