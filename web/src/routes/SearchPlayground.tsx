...
export function SearchPlayground() {
  ...
  return <>
    <div className="page-header"><div><span className="eyebrow">HYBRID RETRIEVAL</span><h1>Search</h1><p>Dense and lexical candidates with trust, validity, filters, and a silence threshold.</p></div></div>
    ...
    <Card style={{ marginTop: 16 }}>
      ...
      {search.data && !search.data.memories.length ? <EmptyState title="No reliable match" body="The policy abstained instead of filling the answer with weak results." /> : search.data?.memories.map((memory) => <article className="list-row" key={memory.id}><span className="list-primary"><strong>{memory.text}</strong><span>score {memory.score.toFixed(3)} · dense {memory.similarity.toFixed(3)} · lexical {memory.lexical.toFixed(3)}</span></span><Badge>{memory.kind}</Badge><Badge>{memory.source_role}</Badge><span className="page-actions" aria-label="Result feedback"><Button variant="secondary" size="sm" disabled={feedback.isPending} onClick={() => feedback.mutate({ memoryId: memory.id, useful: true, correct: true })}><Check size={13} /> Useful</Button><Button variant="secondary" size="sm" disabled={feedback.isPending} onClick={() => feedback.mutate({ memoryId: memory.id, useful: false, correct: false })}><X size={13} /> Wrong</Button></span></article>)}
      {search.data ? <div className="pagination"><span>{search.data.used_tokens} tokens · {search.data.policy_id} · {search.data.timings.total_ms} ms</span></div> : null}
    </Card>
  </>;
}
